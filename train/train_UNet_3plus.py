...
from src.UNet_3plus.UNet_3Plus import UNet_3Plus
...
def train():
    ...
    for fold in range(kfold):
        ...
        seg_model = UNet_3Plus()  # Model initialization
        opt = optimizer(seg_model.parameters(), lr=lr, weight_decay=weight_decay)
        seg_model.to(device)
        metric.to(device)

        # Define the path to save the model weights
        model_name = os.path.join(save_path, str(fold+1), 'Unet_3plus.pth')
        max_score = 0
        # Compute and print total number of parameters in the model
        ...
        for epoch in range(epochs):
            ...
            if max_score < val_logs['Dice']:
                ...
                print('Model saved!!!')
                epoch_time = time.time() - start_train_time  # End time for the current epoch
