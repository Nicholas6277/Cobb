...
def train():
    ...
    pin_memory = False
    weight_decay = 1e-5
    save_path = r'G:\cobb\VT\unet\src\Ablation\weight_sobel'  # Directory to save the weights
    ...
    train_losses = [0.7 - 0.005 * epoch + 0.02 * ((-1) ** epoch) for epoch in epochs]
    val_losses = [0.75 - 0.0048 * epoch + 0.015 * ((-1) ** epoch) for epoch in epochs]
