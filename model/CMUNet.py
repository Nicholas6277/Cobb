...
class ConvMixerBlock(nn.Module):
    def __init__(self, dim=512, depth=7, k=5):
...
class CMUNet(nn.Module):
    def __init__(self, img_ch=1, output_ch=1, l=7, k=7):
        """
        Args:
        ...
        self.ConvMixer = ConvMixerBlock(dim=192, depth=l, k=k)
        # Decoder
        self.Up5 = up_conv(ch_in=192, ch_out=96)
        self.Up_conv5 = conv_block(ch_in=96* 2, ch_out=96)
        self.Up4 = up_conv(ch_in=96, ch_out=48)
        self.Up_conv4 = conv_block(ch_in=48 * 2, ch_out=48)
        self.Up3 = up_conv(ch_in=48, ch_out=24)
        self.Up_conv3 = conv_block(ch_in=24 * 2, ch_out=24)
        self.Up2 = up_conv(ch_in=24, ch_out=12)
        self.Up_conv2 = conv_block(ch_in=12 * 2, ch_out=12)
        self.Conv_1x1 = nn.Conv2d(12, output_ch, kernel_size=1, stride=1, padding=0)
        # Skip-connection
        self.msag4 = MSAG(96)
        self.msag3 = MSAG(48)
        self.msag2 = MSAG(24)
