...
import sys
from utils import AverageValueMeter, Dice, to_string  # Assuming utils.py has the required functions
from dataset import AugmentedImageDataset  # Assuming dataset.py contains your Dataset class
from dataset import get_complex_training_augmentation
from dataset import get_validation_augmentation
from loss import HybridLoss
from src.resunetplus.res_unet_plus import ResUnetPlusPlus



from modelswithWT import myModel_1  # Assuming the model is defined in model.py
...
def train():
    ...
    pin_memory = False
    weight_decay = 1e-5
    save_path = 'F:/cobb/VT/unet/weight/'  # Directory to save the weights
    ...
    def get_train_valid_data(fold):
        ...
        train_images = glob.glob(f'F:/cobb/VT/unet/data/f0{(fold+1)%3+1}/image/*.png') + \
                      glob.glob(f'F:/cobb/VT/unet/data/f0{(fold+2)%3+1}/image/*.png')
        train_labels = glob.glob(f'F:/cobb/VT/unet/data/f0{(fold+1)%3+1}/label/*.png') + \
                      glob.glob(f'F:/cobb/VT/unet/data/f0{(fold+2)%3+1}/label/*.png')

        valid_images = glob.glob(f'F:/cobb/VT/unet/data/f0{fold%3+1}/image/*.png')
        valid_labels = glob.glob(f'F:/cobb/VT/unet/data/f0{fold%3+1}/label/*.png')
    ...
    for fold in range(kfold):
        ...
        seg_model = ResUnetPlusPlus(1)  # Model initialization
        opt = optimizer(seg_model.parameters(), lr=lr, weight_decay=weight_decay)
        seg_model.to(device)
        metric.to(device)

        # Define the path to save the model weights
        model_name = os.path.join(save_path, str(fold+1), 'resnetplus.pth')
        max_score = 0
