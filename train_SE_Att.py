...
def train():
    ...
    pin_memory = False
    weight_decay = 1e-5
    save_path = r'G:\cobb\VT\unet\src\Ablation\weight'  # Directory to save the weights
    ...
    def get_train_valid_data(fold):
        ...
        train_images = glob.glob(f'G:/cobb/VT/unet/data/f0{(fold+1)%3+1}/image/*.png') + \
                      glob.glob(f'G:/cobb/VT/unet/data/f0{(fold+2)%3+1}/image/*.png')
        train_labels = glob.glob(f'G:/cobb/VT/unet/data/f0{(fold+1)%3+1}/label/*.png') + \
                      glob.glob(f'G:/cobb/VT/unet/data/f0{(fold+2)%3+1}/label/*.png')

        valid_images = glob.glob(f'G:/cobb/VT/unet/data/f0{fold%3+1}/image/*.png')
        valid_labels = glob.glob(f'G:/cobb/VT/unet/data/f0{fold%3+1}/label/*.png')
