...
class Attention_block(nn.Module):
    def __init__(self, F_g, F_l, F_int):
        ...
        self.W_g = nn.Sequential(
            ...
            WTConv2d(F_int, F_int, kernel_size=3, wt_levels=3),
        ...
        self.psi = nn.Sequential(
            ...
            WTConv2d(1, 1, kernel_size=3, wt_levels=3),
            nn.BatchNorm2d(1),
            nn.Sigmoid()  # 生成权重矩阵
        )
...
class up(nn.Module):
    def __init__(self, chanel_in, chanel_out, stride=1, padding=1, bilinear=True):  # bilinear：双线性插值进行上采样
        super().__init__()
        if bilinear:
            ...
            self.conv = nn.Sequential(
                ...
                WTConv2d(chanel_out, chanel_out, kernel_size=3, wt_levels=3),
                WTConv2d(chanel_out, chanel_out, kernel_size=3, wt_levels=3),
            )
        else:
        ...
        self.AG = Attention_block(chanel_in, chanel_out, chanel_out)
